...
namespace FIAP.CP2.API.Controllers
{
    ...
    public class ProfissaoAtualController : ControllerBase
    {
        ...
        public IActionResult GetById(int id)
        {
            ...
            if (result == null)
                return NotFound("Profissão atual não encontrada.");

            return Ok(result);
        }

        [HttpPost]
        public IActionResult Create([FromBody] ProfissaoAtualDTO dto)
        {
        ...
        [HttpPut("{id}")]
        public IActionResult Update(int id, [FromBody] ProfissaoAtualDTO dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
