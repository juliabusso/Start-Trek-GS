...
namespace FIAP.CP2.API.Controllers
{
    ...
    public class ProfissaoAntigaController : ControllerBase
    {
        ...
        public IActionResult GetById(int id)
        {
            ...
            if (result == null)
                return NotFound("Profissão antiga não encontrada.");

            return Ok(result);
        }

        [HttpPost]
        public IActionResult Create([FromBody] ProfissaoAntigaDTO dto)
        {
        ...
        [HttpPut("{id}")]
        public IActionResult Update(int id, [FromBody] ProfissaoAntigaDTO dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
